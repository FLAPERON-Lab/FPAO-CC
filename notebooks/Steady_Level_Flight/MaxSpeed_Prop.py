...
def _():
    def maxthrust_solver(W, h, Model):
        ...
        function = lambda CL: (
            Model.aircraft.Pa0 * 1e3 * sigma**Model.aircraft.beta
            - W**1.5
            / (sigma**0.5)
            * np.sqrt(2 / atmos.rho0 / Model.aircraft.S)
            * (Model.aircraft.CD0 + Model.aircraft.K * CL**2)
            / (CL ** (3 / 2))
        )
